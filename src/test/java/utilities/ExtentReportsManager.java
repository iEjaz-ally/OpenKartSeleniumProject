...
public class ExtentReportsManager implements ITestListener {
	...
	public void onTestSuccess(ITestResult result) {
		...
		testsExtentTest = extentReports.createTest(result.getTestClass().getName());
...
public void onTestFailure(ITestResult result) {
		...
		testsExtentTest = extentReports.createTest(result.getTestClass().getName());
		testsExtentTest.assignCategory(result.getMethod().getGroups());
		testsExtentTest.log(Status.INFO, "Test Failed");
		testsExtentTest.log(Status.FAIL, result.getThrowable().getMessage());
		try {
			testsExtentTest.addScreenCaptureFromPath(DriverUtility.takeScreenShot());
		}
		catch (Exception e) {
			e.printStackTrace();
		}
...
public void onFinish(ITestContext testContext) {
	...
	File file = new File(System.getProperty("user.dir")+File.separator+ "Reports"+File.separator+repNameString);
	try {
